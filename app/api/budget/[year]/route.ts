...
export async function GET(
  ...
  const months = monthFields.map((field, idx) => ({
    month: idx + 1,
    budget: lines.reduce((s, l) => s + ((l as Record<string, number>)[field] ?? 0), 0),
  }))

  const totalAnnual = lines.reduce((s, l) => s + (l.total_annual ?? 0), 0)
  const categoryA = lines.filter(l => l.category === "A").reduce((s, l) => s + (l.total_annual ?? 0), 0)
  const categoryB = lines.filter(l => l.category === "B").reduce((s, l) => s + (l.total_annual ?? 0), 0)
